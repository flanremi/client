...
# 登录的设备信息
# DEV_IP = create_string_buffer(b'169.254.14.79')
DEV_IP = create_string_buffer(b'192.168.50.252')
DEV_PORT = 8000
...
def start_camera(ip: str):
    ...
    global win
    global DEV_IP
    DEV_IP = create_string_buffer(bytes(ip, "utf-8"))
    # 创建窗口
    win = tkinter.Tk()
    # 固定窗口大小
    win.resizable(0, 0)
    ...
    # 创建退出按键
    b1 = Button(win, text='退出', command=w_quit)
    ...
    if WINDOWS_FLAG:
        os.chdir(
            r'E:\\java_project\\hikvision\\CH-HCNetSDKV6.1.9.4_build20220412_win64\\Demo示例\\5- Python开发示例\\1-预览取流解码Demo\\lib\\win')
...
# docker run -d -p 4995:5000 -v /home/dxh/cdn/:/usr/src/app/file flanremi/file_server:v3 flask run --host=0.0.0.0
