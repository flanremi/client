                ...
                while True:
                    try:
                        ...
                        if self.in_control == 1:
                            continue
                        results = json.loads(
                            requests.post(url="http://192.168.50.205:5000/yolo_get", data={"camera": "c1"}).text)
                        boxes = []
                        print(results)
...
# CameraManager(ip="192.168.50.25" + "1", yolo_on=False, position=1).display()